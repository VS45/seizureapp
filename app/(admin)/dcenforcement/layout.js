...
export default function CreatorLayout({ children }) {
    ...
    const navItems = [
        { name: 'Dashboard', href: '/dcenforcement/dashboard', icon: <FiHome size={20} /> },
        { name: 'Seizures', href: '/dcenforcement/seizures', icon: <FiFileText size={20} /> },
    ]
    ...
    const fetchUserData = async () => {
        try {
            ...
            if (userData.user.role !== 'dcenforcement') {
                setError('You do not have permission to access dcenforcement pages');
                setLoading(false);
